...
def apply_transformations(func):
    def decorator(x, y, z):
        ...
        def garis_transformasi(m, sy):
            return lambda x: m * x + sy

        return translasi, rotasi, dilatasi, garis_transformasi
...
translasi, rotasi, dilatasi, garis_transformasi = hasil_transformasi(x_input, y_input, z_input)
...
skala_input = float(input("Masukkan nilai skala untuk dilatasi: "))
...
hasil_dilatasi = dilatasi(skala_input)

print("\nHasil Translasi:", hasil_translasi)
print("Hasil Rotasi:", hasil_rotasi)
print("Hasil Dilatasi:", hasil_dilatasi)

print("\nHasil Garis Transformasi:")
for x_value in range(0, 5):
    hasil_garis = garis_transformasi(m_input, sy_input)(x_value)
    print(f"For x = {x_value}, y = {hasil_garis:.2f}")

contoh_garis = f"y = {m_input:.2f}x + {sy_input:.2f}"
